...
<div class="container mx-auto px-6">
    ...
    <form action="{{ route('announcements.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="title" name="title" class="w-full p-2 border rounded" value="{{ old('title') }}" required>
        </div>

        <div class="mb-4">
            <label for="description" class="block text-gray-700">Description</label>
            <textarea id="description" name="description" class="w-full p-2 border rounded" rows="5" required>{{ old('description') }}</textarea>
        </div>

        <div class="mb-4">
            <label for="published_at" class="block text-gray-700">Published Date (Optional)</label>
            <input type="date" id="published_at" name="published_at" class="w-full p-2 border rounded" value="{{ old('published_at') }}">
        </div>
